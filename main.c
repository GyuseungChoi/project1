...
int main(){
	...
	int menu;
	while(1){
		printf("\nMenu : 1.Create 2.Read 3.Update 4.Delete 5.list 6.Search(category) 7.Search(evaluation) 8.Search(price_range) 9.Load 10.Save 11.make_report 12.sort by price 0.끝내기> ");
		scanf("%d", &menu);
#ifdef DEBUG
...
void make_report(){
	...
	char* category[]={"치킨","피자","햄버거","족발/보쌈","한식","일식","중식","양식","디저트"};
	printf("category\n");
	fprintf(f, "category\n");
	for(int i=0; i<9; i++){
		d_make_report_category(f, category[i]);
		printf("\n");
	}
...
void sort_by_price(){
	d_sort_price();
	printf("All records are sorted by price.\n");
}
